...
function Actions({ isNewInvoice, isEditingExisting, onSaveDraft, onSaveAndSend, onDiscard, onEdit, onCancelEdit, onSaveChanges, onDelete, onMarkAsPaid, currentStatus, onDownloadPDF, onPrint, onSendDraft, onOpenSendOptions }) {
  ...
  return (
    <section className="actions-section">
      ...
      {currentStatus === 'pending' && (
        <>
          <button className="button-secondary button-icon" onClick={() => { /* Call prop for email modal/action */ alert('Email action for pending'); }} aria-label="Email Invoice">
            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
          ...
          {navigator.share && (
            <button className="button-secondary button-icon" onClick={() => { /* Call prop for share modal/action */ alert('Share action for pending'); }} aria-label="Share Invoice">
              <svg width="16" height="16" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
